...
public class RssHandler extends DefaultHandler {
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        ...
        if (currentItem != null) {
            ...
            if (parsingTitle) currentItem.setTitle(new String(ch, start, length));
                //If parsingLink is true, then that means we are inside a <link> tag so the text is the link of an item.
            else if (parsingLink) currentItem.setLink(new String(ch, start, length));
            else if (parsingEnclosure) currentItem.setEnclosure(new String(ch, start, length));
